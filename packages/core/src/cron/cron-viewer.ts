...
export class CronViewer extends EventEmitter {
  ...
  async cancel(id: string): Promise<void> {
    ...
    try {
      await this.prisma.$executeRawUnsafe('DELETE FROM cron_events WHERE id = $1', id);
    } catch {}
  }
  ...
  private calculateNextRun(cronExpression: string): Date | null {
    try {
      ...
      if (hour !== '*') {
        ...
        if (
          next.getHours() > h ||
          (next.getHours() === h && next.getMinutes() > parseInt(minute, 10)) ||
          0
        ) {
